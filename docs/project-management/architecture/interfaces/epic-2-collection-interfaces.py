...
class BaseDataCollector(ABC):
    ...
    def __init__(
        ...
        self._metrics: CollectionMetrics = CollectionMetrics(
            source=self.get_source_name(), zipcode="", start_time=datetime.now(UTC)
        )
    ...
    async def collect_zipcode(self, zipcode: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                "Collection failed",
                extra={"source": self.get_source_name(), "zipcode": zipcode, "error": str(e)},
            )
...
class CollectionStrategy(Protocol):
    ...
    async def collect_multiple_zipcodes(
        self, collectors: List[DataCollector], zipcodes: List[str]
...
class CollectorFactory(ABC):
    ...
    async def create_collector(
        source_type: DataSourceType, config: ConfigProvider, repository: PropertyRepository
    ) -> DataCollector:
        """
    ...
    async def validate_collector_config(
        source_type: DataSourceType, config: ConfigProvider
    ) -> bool:
...
class CollectionMonitor(Protocol):
    ...
    async def record_collection_start(
        self, source: str, zipcode: str, expected_count: Optional[int] = None
    ) -> None:
    ...
    async def record_collection_error(
        self, source: str, zipcode: str, error: Exception, context: Dict[str, Any]
    ) -> None:
        """Record collection error for analysis."""
        ...

    async def record_rate_limit_event(
        self, source: str, wait_time_seconds: float, requests_made: int
    ) -> None:
...
class DataQualityValidator(Protocol):
    ...
    async def validate_collection_batch(self, properties: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
...
class DataCollectionError(BasePhoenixException):
    ...
    def __init__(
        self, message: str, *, source: Optional[str] = None, zipcode: Optional[str] = None, **kwargs
    ):
...
class Epic2ServiceProvider(ABC):
    ...
    @abstractmethod
    async def create_collector(
        self, source_type: DataSourceType, config: ConfigProvider, repository: PropertyRepository
    ) -> DataCollector:
