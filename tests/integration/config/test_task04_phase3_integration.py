...
class TestTask04Phase3ConfigurationIntegration:
    ...
    def integration_config_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            testing_config = {
                'logging': {
                    'level': 'DEBUG',
                    'file_path': '/tmp/phoenix_integration_test.log'
                },
                'database': {
                    'name': 'phoenix_real_estate_integration_test',
                    'max_pool_size': 3
                },
                'collection': {
    ...
    async def test_multi_environment_configuration_integration(self, integration_config_dir):
        ...
        for env_name, expected_log_level, expected_db_name, expected_rate_limit in environments_to_test:
            # Update environment
            ...
            log_level = config.get('logging.level')
            assert log_level in [expected_log_level, 'DEBUG'], f"Environment {env_name}: expected {expected_log_level}, got {log_level}"
            
            db_name = config.get('database.name')
            expected_db = expected_db_name if env_name != 'testing' else 'phoenix_real_estate_integration_test'  # Override from env var
            ...
            if env_name == 'production':
                ...
                assert len(critical_errors) <= 3, f"Too many critical errors in {env_name}: {critical_errors}"
    ...
    def _extract_maricopa_config(self, config: EnvironmentConfigProvider) -> Dict[str, Any]:
        ...
        return {
            ...
            'base_url': config.get('maricopa_base_url', config.get('sources.maricopa_county.base_url')),
            'timeout': config.get_typed('maricopa_timeout', int, 
    ...
    def _extract_rate_limiter_config(self, config: EnvironmentConfigProvider) -> Dict[str, Any]:
        ...
        return {
            'max_requests_per_hour': config.get_typed('maricopa_rate_limit', int,
    ...
    async def test_performance_and_security_integration(self, integration_config_dir):
        ...
        assert second_access_time < first_access_time or second_access_time < 0.001  # Cache should be faster
        
        # Test security validation integration
        validation_errors = config.validate()
        
        # Check for security-related validation
        security_errors = [error for error in validation_errors 
                          if any(keyword in error.lower() for keyword in ['secret', 'password', 'key', 'security'])]
    ...
    async def test_configuration_helper_methods_integration(self, integration_config_dir):
        ...
        minimal_config = EnvironmentConfigProvider(
            config_dir=integration_config_dir / "nonexistent",
            environment='minimal'
        )
    ...
    async def test_end_to_end_validation_integration(self, integration_config_dir):
        ...
        missing_errors = [error for error in validation_errors_missing 
                         if 'missing' in error.lower() and 'required' in error.lower()]
