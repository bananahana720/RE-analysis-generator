...
@pytest.mark.skipif(
    not os.environ.get("MONGODB_URI"), reason="MONGODB_URI not set - skipping integration tests"
)
class TestDatabaseConnectionIntegration:
    ...
    async def test_real_connection(self, mongodb_uri, test_database_name):
        ...
        db_conn = DatabaseConnection.get_instance(mongodb_uri, test_database_name)
    ...
    async def test_index_creation(self, mongodb_uri, test_database_name):
        ...
        async with db_conn.get_database() as db:
            ...
            for expected_key in compound_indexes:
                found = any(idx.get("key") == expected_key for idx in properties_indexes.values())
                assert found, f"Compound index {expected_key} not found"

            # Verify daily_reports date unique index
            date_index = next(
                (idx for idx in daily_reports_indexes.values() if idx.get("key") == [("date", 1)]),
                None,
            )
        ...
        async def perform_operation(operation_id: int):
            ...
            async with db_conn.get_database() as db:
                ...
                result = await db["test_concurrent"].insert_one(
                    {"operation_id": operation_id, "timestamp": datetime.now(timezone.utc)}
                )
                assert result.inserted_id is not None

                # Read it back
                doc = await db["test_concurrent"].find_one({"_id": result.inserted_id})
    ...
    async def test_connection_pool_limits(self, mongodb_uri, test_database_name):
        ...
        db_conn = DatabaseConnection.get_instance(
            mongodb_uri, test_database_name, max_pool_size=3, min_pool_size=1
        )
    ...
    async def test_database_operations(self, mongodb_uri, test_database_name):
        ...
        async with db_conn.get_database() as db:
            ...
            # Find document
            found_doc = await db[test_collection].find_one({"property_id": "test-123"})
            assert found_doc is not None
            assert found_doc["address"]["street"] == "123 Test St"

            # Update document
            update_result = await db[test_collection].update_one(
                {"property_id": "test-123"}, {"": {"current_price": 275000}}
            )
            assert update_result.modified_count == 1

            # Verify update
            updated_doc = await db[test_collection].find_one({"property_id": "test-123"})
            assert updated_doc["current_price"] == 275000

            # Delete document
            delete_result = await db[test_collection].delete_one({"property_id": "test-123"})
