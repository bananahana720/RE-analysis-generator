...
class TestLoggingConfigurationIntegration:
    ...
    def test_environment_specific_logging(self):
        ...
        environments = {
            "development": {"format": "text", "expected_colors": True},
            "production": {"format": "json", "expected_colors": False},
        }
...
class TestLoggingPerformance:
    ...
    def test_logging_with_large_context(self):
        ...
        large_context = {f"field_{i}": f"value_{i}" * 10 for i in range(100)}
    ...
    def test_logging_filtering_performance(self):
        ...
        sensitive_context = {
            **{f"password_{i}": f"secret_{i}" for i in range(50)},
            **{f"api_key_{i}": f"key_{i}" for i in range(50)},
            **{f"token_{i}": f"token_{i}" for i in range(50)},
            **{f"safe_field_{i}": f"value_{i}" for i in range(50)},
        }
