...
class TestProcessingPipelineE2E:
    ...
    async def test_batch_html_processing(
        self, processing_pipeline, test_data_factory, performance_tracker
    ):
        ...
        async with processing_pipeline:
            results = await processing_pipeline.process_batch_html(
                html_contents, "phoenix_mls", timeout=30, strict_validation=True
            )
        ...
        performance_tracker.record(
            "batch_html_processing", duration, success_count == len(html_contents)
        )
    ...
    async def test_batch_json_processing(
        ...
        async with processing_pipeline:
            results = await processing_pipeline.process_batch_json(
                json_items, "maricopa_county", timeout=30, strict_validation=True
            )
        ...
        success_count = sum(1 for r in results if r.is_valid)
        performance_tracker.record(
            "batch_json_processing", duration, success_count == len(json_items)
        )
    ...
    async def test_mixed_batch_processing(
        self, processing_pipeline, sample_batch_data, performance_tracker
    ):
        ...
        async with processing_pipeline:
            results = await processing_pipeline.process_mixed_batch(
                sample_batch_data, timeout=30, strict_validation=True
            )
        ...
        success_count = sum(1 for r in results if r.is_valid)
        performance_tracker.record(
            "mixed_batch_processing", duration, success_count == len(sample_batch_data)
        )
        ...
        assert any(r.is_valid for r in html_results), (
            "No Phoenix MLS properties processed successfully"
        )
        assert any(r.is_valid for r in json_results), (
            "No Maricopa properties processed successfully"
        )
    ...
    async def test_concurrent_processing_limits(
        ...
        html_contents = [
            test_data_factory.create_phoenix_mls_html(
                address=f"{i} Concurrent St", mls_number=f"675433{i}"
            )
            for i in range(10)
        ]
        ...
        async with processing_pipeline:
            results = await processing_pipeline.process_batch_html(
                html_contents, "phoenix_mls", timeout=30
            )
    ...
    async def test_error_handling_invalid_html(self, processing_pipeline, performance_tracker):
        ...
        async with processing_pipeline:
            with pytest.raises(ProcessingError):
                await processing_pipeline.process_html(
                    invalid_html, "phoenix_mls", timeout=10, strict_validation=True
                )

    async def test_error_handling_invalid_source(self, processing_pipeline):
        """Test error handling for invalid data source."""
        # Act & Assert
        async with processing_pipeline:
            with pytest.raises(ValueError, match="Unsupported source"):
                await processing_pipeline.process_html(
                    "<html></html>", "invalid_source", timeout=10
                )

    async def test_timeout_handling(self, processing_pipeline, mock_ollama_client, e2e_mode):
    ...
    async def test_metrics_tracking(
        ...
        async with processing_pipeline:
            ...
            # Process batch
            await processing_pipeline.process_mixed_batch(sample_batch_data, timeout=30)
    ...
    async def test_performance_benchmarks(
        self, processing_pipeline, test_data_factory, performance_tracker, e2e_mode
    ):
        ...
        async with processing_pipeline:
            for size in test_sizes:
                ...
                html_contents = [
                    test_data_factory.create_phoenix_mls_html(
                        address=f"{i} Perf Test St", mls_number=f"6754{i:03d}"
                    )
                    for i in range(size)
                ]
...
class TestProcessingIntegratorE2E:
    ...
    async def test_integrator_single_property_flow(
        self, processing_integrator, sample_phoenix_mls_html, test_repository, performance_tracker
    ):
        """Test complete flow from HTML to database storage."""
        # Arrange
        property_data = {"html": sample_phoenix_mls_html, "property_id": "TEST-MLS-001"}

        start_time = time.time()

        # Act
        async with processing_integrator:
            result = await processing_integrator.process_property(property_data, "phoenix_mls")
    ...
    async def test_integrator_batch_processing(
        self, processing_integrator, test_data_factory, test_repository, performance_tracker
    ):
        ...
        batch_data = [
            {
                "html": test_data_factory.create_phoenix_mls_html(
                    address=f"{i} Integration St", mls_number=f"INT{i:03d}"
                ),
        ...
        # Act
        async with processing_integrator:
            batch_result = await processing_integrator.process_batch(batch_data, "phoenix_mls")

        # Record performance
        duration = time.time() - start_time
        performance_tracker.record(
            "integrator_batch_processing", duration, batch_result.successful == len(batch_data)
        )
    ...
    async def test_integrator_metrics(self, processing_integrator, sample_batch_data):
        ...
        async with processing_integrator:
            ...
            for item in sample_batch_data:
                await processing_integrator.process_property(item, item["source"])
    ...
    @pytest.mark.slow
    async def test_integrator_error_recovery(self, processing_integrator, test_data_factory):
        """Test integrator's error recovery capabilities."""
        # Arrange
        # Mix valid and invalid data
        mixed_data = [
            {"html": test_data_factory.create_phoenix_mls_html(), "property_id": "VALID-001"},
            {"html": "<invalid>no data here</invalid>", "property_id": "INVALID-001"},
            {
                "html": test_data_factory.create_phoenix_mls_html(address="789 Recovery Rd"),
        ...
        # Act
        async with processing_integrator:
            batch_result = await processing_integrator.process_batch(mixed_data, "phoenix_mls")
...
async def test_complete_e2e_workflow(
    test_config, test_repository, test_data_factory, performance_tracker, e2e_mode
):
