...
from phoenix_real_estate.collectors.base.rate_limiter import (
    RateLimiter
)
...
class TestRateLimiter:
    ...
    async def test_sliding_window_cleanup(self):
        ...
        limiter = RateLimiter(requests_per_minute=5, safety_margin=0.0, window_duration=2)  # 2 second window
        source = "test_source"
    ...
    def test_get_current_usage_single_source(self):
        ...
        expected_keys = {
            "source", "current_requests", "effective_limit", "requests_remaining",
            "utilization_percent", "is_rate_limited", "next_available_seconds",
            "window_duration"
        }
    ...
    def test_get_current_usage_all_sources(self):
        ...
        expected_keys = {
            "total_sources", "total_current_requests", "effective_limit_per_source",
            "sources", "observers_count", "window_duration", "safety_margin_percent"
        }
        assert set(usage.keys()) == expected_keys
    ...
    def test_get_performance_metrics(self):
        ...
        expected_keys = {
            "configured_limit", "effective_limit", "safety_margin_percent",
            "window_duration_seconds", "total_active_sources", "total_current_requests",
            "observer_count", "most_active_source", "max_source_requests",
            "max_source_utilization_percent", "memory_efficient", "cleanup_working"
        }
    ...
    async def test_performance_response_time(self):
        ...
        assert response_time_ms < 100, f"Response time {response_time_ms:.1f}ms exceeds 100ms requirement"
    ...
    def test_logging_integration(self):
        ...
        with patch('phoenix_real_estate.collectors.base.rate_limiter.get_logger') as mock_get_logger:
            mock_logger = Mock()
            ...
            mock_get_logger.assert_called_once_with('phoenix_real_estate.collectors.base.rate_limiter')
