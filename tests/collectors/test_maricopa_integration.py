...
class TestEpic1FoundationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_property_repository_integration(self, real_property_repository, sample_property_data):
    ...
    def test_logging_framework_integration(self, real_logger):
        ...
        logger.info("Integration test started", extra={
            "component": "maricopa_collector",
            "operation": "integration_test"
        })
        
        logger.warning("Test warning with context", extra={
        ...
        try:
            ...
            logger.error("Test error logging", exc_info=True, extra={
...
class TestEndToEndWorkflowIntegration:
    ...
    async def test_property_adaptation_e2e(self, integrated_collector, mock_api_responses):
        ...
        with patch('phoenix_real_estate.collectors.maricopa.collector.DataValidator') as mock_validator_class:
    ...
    async def test_error_handling_e2e(self, integrated_collector):
        ...
        collector.client.search_by_zipcode = AsyncMock(return_value=[
            {"property_id": "test_001", "invalid": "data"}
        ])
...
class TestComponentIntegrationTesting:
    ...
    def integrated_rate_limiter(self):
        ...
        return RateLimiter(requests_per_minute=600, safety_margin=0.10)  # 600 per minute = 10 per second
    ...
    def integrated_api_client(self, integrated_rate_limiter):
        ...
        config.get_typed.side_effect = lambda key, type_class, default=None: {
            "MARICOPA_TIMEOUT": 30,
            "MARICOPA_RATE_LIMIT": 600  # 10 per second as requests per minute
        }.get(key, default) if type_class == int else default
...
class TestPerformanceRateLimitingIntegration:
    ...
    def performance_collector(self):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            mock_response.status = 200
            mock_response.json = AsyncMock(return_value=[
                {"property_id": f"perf_test_{i}", "data": "test"} for i in range(10)
            ])
            mock_session.get = AsyncMock(return_value=mock_response)
            mock_session.close = AsyncMock()
            mock_session_class.return_value = mock_session
    ...
    async def test_concurrent_collection_thread_safety(self, performance_collector):
        ...
        for i in range(5):
            concurrent_operations.append(
                collector.collect_zipcode(f"8500{i}")
            )
        ...
        concurrent_rate_operations = [
            rate_limited_operation() for _ in range(50)
        ]
...
class TestDataIntegritySchemaValidation:
    ...
    def test_concurrent_validation_thread_safety(self, validation_test_data):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(validate_property_thread, i)
                for i in range(20)
            ]
        ...
        assert successful_validations == 20, f"Only {successful_validations}/20 validations succeeded"
    ...
    async def test_integration_performance_benchmarks(self):
        ...
        with patch.object(config, 'get') as mock_get, \
            ...
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                logger.info("Integration benchmarks completed", extra={
