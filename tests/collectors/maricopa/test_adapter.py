...
class TestMaricopaDataAdapter:
    ...
    def sample_raw_data(self):
        """Sample Maricopa API response data for testing based on real API structure."""
        return {
            "apn": "123-45-678",
            "parcel_id": "12345678",
    ...
    def test_price_extraction_multiple_types(self, adapter):
        ...
        prices = adapter._extract_prices(valuation_data)
    ...
    def test_features_extraction_safe_conversions(self, adapter):
        ...
        features = adapter._extract_features(residential_data)
    ...
    def test_tax_info_extraction(self, adapter):
        ...
        raw_data = {
            "apn": "456-78-901",
            "valuation": {"assessed_value": 275000, "tax_amount": 3250, "tax_year": 2024},
        }
    ...
    def test_quality_score_calculation_minimal_data(self, adapter, minimal_raw_data):
        ...
        assert (
            0.1 <= score <= 0.8
        )  # Should be lower with minimal data, but minimal_raw_data still has decent coverage
    ...
    def test_nested_field_extraction(self, adapter):
        ...
        result = adapter._get_nested_field(
            test_data, ["nonexistent", "primary_field", "secondary_field"]
        )
        assert result == "value1"

        # Should skip empty/invalid values
        result = adapter._get_nested_field(
            test_data, ["empty_field", "zero_field", "none_field", "primary_field"]
        )
        assert result == "value1"
...
class TestDataValidator:
    ...
    def test_validate_property_valid(self):
        ...
        address = PropertyAddress(street="123 Main St", city="Phoenix", state="AZ", zipcode="85001")

        property_obj = Property(property_id="test_property_id", address=address)
    ...
    async def test_integration_scenarios(self, integration_test_data):
        ...
        for scenario in integration_test_data:
            ...
            if result.sources:
                ...
                assert abs(quality_score - expected_score) < 0.2, (
                    f"Quality score {quality_score} not near expected {expected_score} for {scenario['name']}"
                )

    @pytest.mark.asyncio
    async def test_malformed_response_handling(self):
        """Test adapter handling of malformed API responses."""
        adapter = MaricopaDataAdapter(logger_name="malformed_test")
        ...
        with pytest.raises(ValidationError, match="Missing or invalid address section"):
            await adapter.adapt_property(malformed_data)

    @pytest.mark.asyncio
    async def test_real_api_structure_compatibility(self):
        """Test adapter with realistic API response structure."""
        adapter = MaricopaDataAdapter(logger_name="compatibility_test")

        # Test with realistic Maricopa API response
        real_api_response = {
            "apn": "123-45-678",
            "property_type": "Residential",
            "situs_address": {
