...
class TestMaricopaAPIClientConfiguration:
    ...
    def test_init_success_with_required_config(self):
        ...
        assert client.base_url == "https://mcassessor.maricopa.gov"
...
class TestMaricopaAPIClientRequests:
    ...
    async def test_search_property_validation(self, client):
        """Test search query validation in search method."""
        # Test empty query
    ...
    async def test_get_parcel_details_validation(self, client):
        """Test APN validation in parcel methods."""
        # Test empty APN
        with pytest.raises(ValidationError, match="APN cannot be empty"):
            await client.get_parcel_details("")

        # Test whitespace-only APN
        with pytest.raises(ValidationError, match="APN cannot be empty"):
            await client.get_parcel_details("   ")

        # Test invalid APN format (too short)
        with pytest.raises(ValidationError, match="Invalid APN format"):
            await client.get_parcel_details("123")
    ...
    async def test_get_property_info_validation(self, client):
        """Test APN validation for property info endpoint."""
        with pytest.raises(ValidationError, match="APN cannot be empty"):
            await client.get_property_info("")

        with pytest.raises(ValidationError, match="APN cannot be empty"):
            await client.get_property_info("   ")

    async def test_get_valuations_validation(self, client):
        """Test APN validation for valuations endpoint."""
        with pytest.raises(ValidationError, match="APN cannot be empty"):
            await client.get_valuations("")

    async def test_get_residential_details_validation(self, client):
        """Test APN validation for residential details endpoint."""
        with pytest.raises(ValidationError, match="APN cannot be empty"):
            await client.get_residential_details("")
    ...
    async def test_search_property_pagination_params(self, client):
        """Test pagination parameter validation for property search."""
        # Test negative page number
        with pytest.raises(ValidationError, match="Page number must be positive"):
            await client.search_property("85001", page=-1)

        # Test zero page number
        with pytest.raises(ValidationError, match="Page number must be positive"):
            await client.search_property("85001", page=0)

        # Test excessive page number
        with pytest.raises(ValidationError, match="Page number cannot exceed 1000"):
            await client.search_property("85001", page=1001)
    ...
    @patch("aiohttp.ClientSession.request")
    async def test_successful_search_property_handling(self, mock_request, client):
        """Test successful search property request handling."""
    ...
    @patch("aiohttp.ClientSession.request")
    async def test_successful_parcel_details_handling(self, mock_request, client):
        """Test successful parcel details request handling."""
        ...
        with patch.object(client.rate_limiter, "wait_if_needed") as mock_wait:
            ...
            with (
                ...
                assert len(sleep_calls) >= 1, (
                    f"Expected sleep(30) to be called, got {mock_async_sleep.call_args_list}"
                )
    ...
    async def test_retry_async_integration(self, client):
        ...
        with patch("aiohttp.ClientSession.request") as mock_request:
            ...
            with patch(
                "phoenix_real_estate.collectors.maricopa.client.retry_async", wraps=real_retry_async
            ) as mock_retry:
                with patch.object(client.rate_limiter, "wait_if_needed") as mock_wait:
                    mock_wait.return_value = 0.0
                    result = await client.search_property("85001")
    ...
    async def test_error_handling_and_logging(self, mock_request, client):
        ...
        with patch.object(client.rate_limiter, "wait_if_needed") as mock_wait:
            ...
            with patch("asyncio.sleep", new_callable=AsyncMock), patch("time.sleep"):
...
class TestMaricopaAPIClientSecurity:
    ...
    async def test_credential_exposure_prevention(self, client):
        ...
        try:
            await client._handle_request_error(Exception("Test error"), "test_operation", **context)
