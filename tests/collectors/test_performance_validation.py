...
class TestPerformanceValidation:
    ...
    async def test_response_time_zipcode_search(self, mock_config):
        ...
        mock_response.json = AsyncMock(return_value={
            "results": [{"property_id": f"test_{i}"} for i in range(100)]
        })
    ...
    def test_memory_usage_during_collection(self, mock_config, mock_repository):
        ...
        mock_client.search_properties.return_value = [
            {"property_id": f"test_{i}", "data": "x" * 1000}
            for i in range(1000)
        ]
    ...
    async def test_connection_management(self, mock_config):
        ...
        print(f"✅ Connection management: {connections_created} created, {connections_closed} closed")
...
if __name__ == "__main__":
    ...
    result = subprocess.run([
        ...
        __file__, 
        "-v", 
        "-k", "test_rate_limiting_compliance or test_memory_usage or test_concurrent"
    ])
