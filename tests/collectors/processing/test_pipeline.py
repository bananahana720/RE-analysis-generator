...
class TestDataProcessingPipeline:
    ...
    def sample_validation_result(self) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            quality_metrics=DataQualityMetrics(
                completeness=0.9, consistency=0.95, accuracy=0.98, timeliness=0.99
            ),
        )
    ...
    async def pipeline(self, test_config, mock_extractor, mock_validator):
        ...
        with patch(
            "phoenix_real_estate.collectors.processing.pipeline.PropertyDataExtractor",
            return_value=mock_extractor,
        ):
            with patch(
                "phoenix_real_estate.collectors.processing.pipeline.ProcessingValidator",
    ...
    @pytest.mark.asyncio
    async def test_process_single_html(
        self,
        pipeline,
        mock_extractor,
        mock_validator,
    ...
    @pytest.mark.asyncio
    async def test_process_single_json(
        self,
        pipeline,
        mock_extractor,
        mock_validator,
    ...
    @pytest.mark.asyncio
    async def test_process_batch_html(
        self,
        pipeline,
        mock_extractor,
        mock_validator,
        ...
        # Setup mocks
        extracted_data = [
            dict(sample_property_data, property_id=f"TEST-{i:03d}") for i in range(10)
        ]
    ...
    @pytest.mark.asyncio
    async def test_process_batch_with_concurrency_limit(
        self,
        pipeline,
        mock_extractor,
        mock_validator,
