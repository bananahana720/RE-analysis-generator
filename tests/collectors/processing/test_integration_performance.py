...
async def test_cache_integration():
    ...
    async with DataProcessingPipeline(config) as pipeline:
        ...
        result1 = await pipeline.process_html("<div>Test property 1</div>", "phoenix_mls")
        assert result1.is_valid
        assert mock_extract.call_count == 1

        # Second call with same content - should hit cache
        result2 = await pipeline.process_html("<div>Test property 1</div>", "phoenix_mls")
    ...
    async with DataProcessingPipeline(config) as pipeline:
        ...
        for batch_num in range(3):
            ...
            if "batch_optimization" in metrics:
                ...
                print(
                    f"Batch {batch_num}: current={current_size}, optimal={optimal_size}, duration={duration:.2f}s"
                )
