        ...
        result = ValidationResult(
            ...
            field_validations={
                "price": FieldValidation(
                    field_name="price", is_valid=True, confidence=0.98, value=350000, errors=[]
                )
            },
            quality_metrics=DataQualityMetrics(
                completeness=0.85, consistency=0.92, accuracy=0.90, timeliness=0.95
            ),
        )
...
class TestProcessingValidator:
    ...
    def test_validate_price_range(self, validator):
        ...
        assert price_validation.confidence < 0.5, (
            f"Price confidence should be low: {price_validation}"
        )
