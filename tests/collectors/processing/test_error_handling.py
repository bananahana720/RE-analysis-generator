...
class TestErrorClassifier:
    ...
    def test_get_recovery_action(self):
        ...
        assert (
            classifier.get_recovery_action(ErrorType.NETWORK) == RecoveryAction.RETRY_WITH_BACKOFF
        )
        ...
        assert (
            classifier.get_recovery_action(ErrorType.AUTHENTICATION)
        ...
        assert (
            classifier.get_recovery_action(ErrorType.TEMPORARY) == RecoveryAction.RETRY_WITH_BACKOFF
        )
...
class TestDeadLetterQueue:
    ...
    async def test_add_failed_item(self):
        ...
        item = {"parcel_number": "123-45-678", "data": {"address": "123 Main St"}}
    ...
    async def test_max_size_enforcement(self):
        ...
        # Add three items
        for i in range(3):
            await dlq.add_failed_item({"id": i}, Exception(f"Error {i}"), attempt_count=1)
    ...
    async def test_export_to_file(self):
        ...
        for i in range(3):
            await dlq.add_failed_item(
                {"parcel_number": f"123-45-{i:03d}"}, Exception(f"Error {i}"), i + 1
            )
...
class TestErrorRecoveryStrategy:
    ...
    async def test_handle_network_error(self):
        ...
        result = await strategy.handle_error(flaky_function, {}, max_retries=3)
    ...
    async def test_handle_rate_limit_error(self):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            with pytest.raises(ProcessingError):
                await strategy.handle_error(rate_limited_function, {}, max_retries=1)
    ...
    async def test_handle_data_error_with_fallback(self):
        ...
        result = await strategy.handle_error(failing_parser, context, use_fallback=True)
    ...
    async def test_circuit_breaker_integration(self):
        ...
        for _ in range(2):
            with pytest.raises(Exception):
                await strategy.handle_error_with_circuit_breaker("test_service", always_fails, {})
    ...
    async def test_dead_letter_queue_integration(self):
        ...
        with pytest.raises(Exception):
            await strategy.handle_error(
                always_fails, {"item": item}, max_retries=2, add_to_dlq=True
            )
    ...
    async def test_comprehensive_error_handling(self):
        ...
        for scenario in test_scenarios:
            ...
            async def test_function():
                ...
                if (
                    scenario["should_succeed_after"] >= 0
                    and call_count > scenario["should_succeed_after"]
                ):
                    return {"success": True}
                raise scenario["error"]

            context = {
                "raw_data": {"description": "Test property at 123 Main St, Phoenix, AZ 85001"}
            }
            ...
            if scenario["should_succeed_after"] == -1:
                ...
                with pytest.raises(ProcessingError):
                    await strategy.handle_error(
                        test_function, context, max_retries=3, use_fallback=True
                    )
            else:
