...
class TestLoadTimeBenchmarks:
    ...
    def test_environment_switch_performance(self):
        ...
        try:
            ...
            # Performance requirements
            assert avg_switch_time < 0.01, (
                f"Average switch time {avg_switch_time:.3f}s exceeds 10ms"
            )
...
class TestConcurrentAccessPerformance:
    ...
    def test_throughput_scaling(self):
        ...
        assert (
            results[-1][1] > results[0][1] * 0.8
        )  # 32 threads should be at least 80% of single thread
...
class TestMemoryEfficiency:
    ...
    def test_memory_overhead_per_config(self):
        ...
        assert memory_per_config < 100 * 1024, (
            f"Config uses {memory_per_config / 1024:.1f}KB per instance"
        )
...
class TestCachePerformance:
    ...
    def test_cache_invalidation_performance(self):
        ...
        # Should be very fast
        assert avg_reset_time < 0.0001, (
            f"Average reset time {avg_reset_time * 1000:.3f}ms exceeds 0.1ms"
        )
...
class TestComprehensiveBenchmark:
    ...
    def test_production_workload_simulation(self):
        ...
        # Metrics
        metrics = {"config_reads": 0, "cache_resets": 0, "secret_reads": 0, "errors": 0}
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=read_threads + write_threads + secret_threads
        ) as executor:
            futures = []
