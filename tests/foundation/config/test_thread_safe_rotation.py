...
class TestAdvancedSecretRotation:
    ...
    def test_atomic_secret_rotation(self):
        ...
        def reader_thread():
            ...
            for _ in range(1000):
                ...
                if (
                    value
                    and value != initial_value
                    and value != "new_user:new_pass@new_host:5432/new_db?ssl=false"
                ):
    ...
    def test_bulk_secret_rotation_performance(self):
        ...
        assert avg_rotation_time < 0.0001, (
            f"Average rotation time {avg_rotation_time * 1000:.3f}ms exceeds 0.1ms"
        )
        assert max_rotation_time < 0.001, (
            f"Max rotation time {max_rotation_time * 1000:.3f}ms exceeds 1ms"
        )
...
class TestMemoryLeakPrevention:
    ...
    def test_thread_local_storage_cleanup(self):
        ...
        def worker_thread(thread_id: int):
            ...
            results.append(
                {"thread_id": thread_id, "config_id": id(config), "environment": config.environment}
            )
