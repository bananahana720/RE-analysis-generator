...
class TestEnhancedTypeConversion:
    ...
    def minimal_config(self, temp_config_dir):
        ...
        config = {"database": {"uri": "mongodb://localhost"}, "logging": {"level": "INFO"}}
    ...
    def test_empty_string_conversions(self, temp_config_dir, minimal_config):
        ...
        with patch.dict(
            os.environ,
            {"PHOENIX_EMPTY_STR": "", "PHOENIX_SPACE_STR": "   ", "PHOENIX_EMPTY_LIST": ""},
        ):
            provider = EnvironmentConfigProvider(config_dir=temp_config_dir, load_dotenv=False)
    ...
    def test_numeric_conversions(self, temp_config_dir, minimal_config):
        ...
        config = {
            ...
            "logging": {"level": "INFO"},
            "numbers": {"int_value": 42, "float_value": 3.14, "large_int": 999999999999},
        }
        config_file = temp_config_dir / "base.yaml"
        with open(config_file, "w") as f:
            yaml.dump(config, f)
    ...
    def test_list_to_string_conversion(self, temp_config_dir, minimal_config):
        ...
        config = {
            "database": {"uri": "mongodb://localhost"},
            "logging": {"level": "INFO"},
            "lists": {"items": ["apple", "banana", "cherry"], "numbers": [1, 2, 3]},
        }
        config_file = temp_config_dir / "base.yaml"
        with open(config_file, "w") as f:
            yaml.dump(config, f)
    ...
    def test_error_context_with_key(self, temp_config_dir, minimal_config):
        ...
        with patch.dict(
            os.environ, {"PHOENIX_BAD_INT": "not_a_number", "PHOENIX_BAD_FLOAT": "not_a_float"}
        ):
            provider = EnvironmentConfigProvider(config_dir=temp_config_dir, load_dotenv=False)
...
class TestComprehensiveValidation:
    ...
    def test_production_specific_validation(self, temp_config_dir):
        ...
        provider = EnvironmentConfigProvider(
            config_dir=temp_config_dir, environment="production", load_dotenv=False
        )
...
class TestConfigurationHelpers:
    ...
    def test_get_database_config_with_components(self, temp_config_dir):
        ...
        db_env_vars = [
            "PHOENIX_DATABASE_HOST",
            "PHOENIX_DATABASE_PORT",
            "PHOENIX_DATABASE_NAME",
            "PHOENIX_DATABASE_USERNAME",
            "PHOENIX_DATABASE_PASSWORD",
            "DATABASE_HOST",
        ]
        original_env = {}
    ...
    def test_get_collection_config(self, temp_config_dir):
        ...
        config = {
            ...
            "collection": {
                ...
                "retry": {"max_retries": 5, "delay": 2.0, "backoff": 1.5},
            },
            "proxy": {
                "enabled": True,
    ...
    def test_environment_check_methods(self, temp_config_dir):
        ...
        # Test development
        provider = EnvironmentConfigProvider(
            config_dir=temp_config_dir, environment="development", load_dotenv=False
        )
        ...
        # Test testing
        provider = EnvironmentConfigProvider(
            config_dir=temp_config_dir, environment="test", load_dotenv=False
        )
        assert provider.is_development() is False
        ...
        # Test aliases
        provider = EnvironmentConfigProvider(
            config_dir=temp_config_dir, environment="prod", load_dotenv=False
        )
        assert provider.is_production() is True
