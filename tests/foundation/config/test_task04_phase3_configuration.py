...
class TestTask04Phase3Configuration:
    ...
    async def test_epic1_config_provider_integration(self, temp_config_dir):
        ...
        with patch('phoenix_real_estate.foundation.config.environment.EnvironmentConfigProvider') as mock_provider:
    ...
    async def test_configuration_integration_with_components(self, temp_config_dir):
        ...
        rate_config = {
            ...
            'window_size': config.get_typed('collection.rate_limiting.window_size_minutes', int, 60)
        }
    ...
    async def test_security_validation(self, temp_config_dir):
        ...
        assert any('secret key must be at least 32 characters' in error.lower() 
                  for error in validation_errors)
        
        # Test with proper secret key
        os.environ['SECRET_KEY'] = 'production-secret-key-32-chars-minimum-length!'
        
        config2 = EnvironmentConfigProvider(
            config_dir=temp_config_dir,
            environment='production'
        )
        
        validation_errors2 = config2.validate()
        secret_errors = [error for error in validation_errors2 
                        if 'secret key' in error.lower()]
