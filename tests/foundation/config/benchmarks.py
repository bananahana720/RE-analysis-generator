...
class BenchmarkSuite:
    ...
    def _benchmark_concurrent_access(self) -> None:
        ...
        self.results.append(result)
        print(f"    Mixed (90% read): {total_reads/duration:,.0f} reads/sec, {total_writes/duration:,.0f} writes/sec")
    ...
    def _benchmark_memory_usage(self) -> None:
        ...
        result = BenchmarkResult(
            name="memory_peak",
            metrics={'unit': 'MB', 'threads': 32},
            raw_data=[peak_memory]
        )
        self.results.append(result)
    ...
    def _benchmark_secret_access(self) -> None:
        ...
        result = BenchmarkResult(
            name="secret_direct_access",
            metrics={'unit': 'seconds'},
            raw_data=direct_times
        )
        self.results.append(result)
        ...
        result = BenchmarkResult(
            name="secret_encrypted_access",
            metrics={'unit': 'seconds'},
            raw_data=encrypted_times
        )
        self.results.append(result)
    ...
    def _benchmark_yaml_parsing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.results.append(result)
            print(f"    Large YAML ({result.metrics['size_bytes']/1024:.1f}KB): {result.mean*1000:.3f}ms")
    ...
    def _benchmark_cache_efficiency(self) -> None:
        ...
        for pattern_name, pattern_func in patterns:
            ...
            self.results.append(result)
            print(f"    {pattern_name}: {hit_ratio*100:.1f}% hit rate, {result.mean*1000:.3f}ms avg")
        ...
        for thread_count in [1, 2, 4, 8, 16, 32, 64]:
            ...
            result = BenchmarkResult(
                ...
                metrics={
                    ...
                    'efficiency': ops_per_sec / (thread_count * 100 / total_time) if total_time > 0 else 0
                },
                raw_data=[avg_op_time]
            )
            self.results.append(result)
            contention_results.append((thread_count, ops_per_sec))
            print(f"    {thread_count:2d} threads: {ops_per_sec:,.0f} ops/sec (efficiency: {result.metrics['efficiency']*100:.1f}%)")
    ...
    def _generate_reports(self) -> None:
        ...
        json_path = self.output_dir / f"benchmark_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(json_path, 'w') as f:
            json.dump(json_report, f, indent=2)
        print(f"  JSON report saved to: {json_path}")
        
        # Markdown report
        md_path = self.output_dir / f"benchmark_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            if 'cold_start' in result.name:
                ...
                if score == 100:
                    summary['strengths'].append(f"Excellent cache performance: {result.mean*1000000:.0f}μs")
                ...
                if score < 50:
                    summary['bottlenecks'].append(f"Limited concurrency: {throughput:.0f} ops/sec with 64 threads")
    ...
    def _generate_markdown_report(self) -> str:
        ...
        report.append(f"CPU: {self.system_info['cpu_count']} cores | Memory: {self.system_info['memory_gb']:.1f} GB")
    ...
    def _print_metric_bar(self, name: str, value: float, max_value: float, unit: str, target: float = None) -> None:
