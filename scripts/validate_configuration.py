...
class ConfigurationValidator:
    ...
    def _add_result(self, success: bool, message: str, severity: str = "error", 
            ...
            if secret_key:
                if self.environment == 'production':
                    if len(secret_key) < 32:
                        ...
                        self._add_result(True, "Production secret key meets length requirements", "info")
                elif self.environment == 'development':
            ...
            if api_keys:
                for service, key in api_keys.items():
                    if len(key) < 20:
                        ...
                        self._add_result(True, f"API key for {service} has reasonable length", "info")
            ...
            if 'uri' in db_config:
                ...
                if uri.startswith(('mongodb://', 'mongodb+srv://')):
                    ...
                    if self.environment == 'development' and 'dev' not in uri.lower():
                        self._add_result(
                            False,
                            "Development should use a development database",
                            "warning"
                        )
                    elif self.environment == 'production' and ('dev' in uri.lower() or 'test' in uri.lower()):
                        self._add_result(
                            False,
                            "Production should not use development/test databases",
                            "error"
                        )
                else:
                ...
                if missing:
                    self._add_result(
                        False,
                        f"Missing database components: {', '.join(missing)}",
                        "error"
                    )
                else:
        ...
        try:
            ...
            if not zip_codes:
                ...
                if invalid_zips:
                    self._add_result(
                        False,
                        f"Invalid ZIP codes: {invalid_zips}",
                        "error"
                    )
            ...
            if max_retries < 1:
                ...
                self._add_result(True, f"Retry policy configured: {max_retries} max retries", "info")
    ...
    def _validate_logging_configuration(self) -> None:
        ...
        try:
            ...
            if log_level not in valid_levels:
                self._add_result(
                    False,
                    f"Invalid log level: {log_level} (valid: {valid_levels})",
                    "error"
                )
            else:
                ...
                if self.environment == 'production' and log_level == 'DEBUG':
                    ...
                    self._add_result(
                        False,
                        f"Log level {log_level} too restrictive for development",
                        "warning"
                    )
                else:
            ...
            if file_path:
                ...
                if not max_bytes:
                    self._add_result(
                        False,
                        "Log rotation max_bytes not configured",
                        "warning"
                    )
                elif max_bytes < 1024 * 1024:  # 1MB
                    self._add_result(
                        False,
                        f"Log rotation size too small: {max_bytes} bytes",
                        "warning"
                    )
                ...
                if not backup_count or backup_count < 1:
                    self._add_result(
                        False,
                        "Log rotation backup_count should be at least 1",
                        "warning"
                    )
        ...
        try:
            ...
            if cache_enabled:
                ...
                if not cache_dir:
                    self._add_result(
                        False,
                        "Cache enabled but no cache directory specified",
                        "error"
                    )
                else:
            ...
            if max_memory < 128:
                self._add_result(
                    False,
                    f"Memory limit very low: {max_memory}MB (may cause issues)",
                    "warning"
                )
            ...
            for config_key, component in test_configs:
                try:
                    ...
                    if value is not None:
                        ...
                        self._add_result(
                            False,
                            f"{component} configuration missing: {config_key}",
                            severity
                        )
                except Exception as e:
                    self._add_result(
                        False,
                        f"{component} configuration error: {e}",
                        "error"
                    )
    ...
    def _validate_production_requirements(self) -> None:
        ...
        try:
            ...
            for key, description in production_requirements.items():
                ...
                if not value:
                    self._add_result(False, f"Missing production requirement: {description}", "error")
                else:
                    self._add_result(True, f"Production requirement satisfied: {description}", "info")
            ...
            if 'dev' in db_uri.lower() or 'localhost' in db_uri:
                self._add_result(
                    False,
                    "Production should not use development/local database",
                    "error"
                )
    ...
    def _validate_testing_setup(self) -> None:
        ...
        try:
            ...
            if not ('test' in db_uri.lower() or 'test' in db_name.lower()):
                self._add_result(
                    False,
                    "Testing environment should use test database",
                    "warning"
                )
            ...
            if rate_limit > 50:
                self._add_result(
                    False,
                    f"Testing rate limit should be low: {rate_limit}",
                    "warning"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Phoenix Real Estate configuration"
    )
    parser.add_argument(
        '--environment',
        help='Environment to validate (development, testing, production)'
    )
    parser.add_argument(
        '--strict',
        action='store_true',
