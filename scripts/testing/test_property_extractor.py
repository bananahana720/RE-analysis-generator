...
async def test_extractor():
    ...
    async with PropertyDataExtractor(config) as extractor:
        ...
        try:
            mls_result = await extractor.extract_from_html(SAMPLE_HTML, source="phoenix_mls")
        ...
        try:
            maricopa_result = await extractor.extract_from_json(
                SAMPLE_JSON, source="maricopa_county"
            )
        ...
        try:
            batch_results = await extractor.extract_batch(
                [SAMPLE_HTML] * 2, source="phoenix_mls", content_type="html"
            )
            print(f"✓ Batch extraction completed: {len(batch_results)} items processed")
            for i, result in enumerate(batch_results):
                if "error" not in result:
                    print(
                        f"  - Item {i + 1}: ${result.get('price', 'N/A'):,} - {result.get('address', {}).get('street', 'N/A')}"
                    )
                else:
