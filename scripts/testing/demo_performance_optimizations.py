...
async def demo_cache_performance():
    ...
    # Create cache
    cache_config = CacheConfig(enabled=True, backend="memory", ttl_hours=24)
...
async def demo_resource_monitoring():
    ...
    # Create monitor
    limits = ResourceLimits(max_memory_mb=1024, max_cpu_percent=80, max_concurrent_requests=5)
...
async def demo_batch_optimization():
    ...
    optimizer = BatchSizeOptimizer(initial_size=5, min_size=1, max_size=50)
    ...
    for i, (size, duration, success, memory) in enumerate(scenarios):
        optimizer.record_batch_performance(
            batch_size=size, duration=duration, success_rate=success, memory_usage_mb=memory
        )
...
async def demo_integrated_pipeline():
    ...
    print(
        f"  Resource monitoring: {'Enabled' if pipeline.resource_monitoring_enabled else 'Disabled'}"
    )
