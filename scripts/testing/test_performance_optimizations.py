...
async def test_cache_performance():
    ...
    async with DataProcessingPipeline(config) as pipeline:
        ...
        await pipeline.process_batch_html(
            [p["html"] for p in properties],
            "phoenix_mls"
        )
        ...
        print(f"Performance improvement: {(no_cache_duration - cache_duration) / no_cache_duration:.1%}")
...
async def test_resource_monitoring():
    ...
    async with DataProcessingPipeline(config) as pipeline:
        ...
        if pipeline._resource_monitor:
            pipeline._resource_monitor.on_alert(
                lambda alert: alerts_received.append(alert)
            )
    ...
    async with benchmark:
        async with DataProcessingPipeline(config) as pipeline:
            ...
            for wave in range(4):
                ...
                results = await pipeline.process_batch_html(
                    [p["html"] for p in wave_properties],
                    "phoenix_mls"
                )
