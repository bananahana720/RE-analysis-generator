...
class WorkflowRunner:
    ...
    async def setup_mock_services(self, services: List[str]) -> Dict[str, Any]:
        ...
        for service in services:
            if service == "mongodb":
                service_configs[service] = {"host": "localhost", "port": 27017, "status": "mocked"}
            elif service == "ollama":
                service_configs[service] = {"host": "localhost", "port": 11434, "status": "mocked"}
    ...
    async def execute_workflow_job(
        self, job_name: str, workflow_data: Dict[str, Any]
    ) -> Dict[str, Any]:
