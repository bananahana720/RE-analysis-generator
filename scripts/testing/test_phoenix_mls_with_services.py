...
class PhoenixMLSServiceTester:
            ...
            async with async_playwright() as p:
                ...
                try:
                    ...
                    if response and response.ok:
                        ...
                        screenshot_path = f"test_phoenix_mls_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                        ...
                        listings = await page.query_selector_all('.listing-card, .property-card, [data-testid="listing"]')
                        if listings:
                        ...
                        print(f"[ERROR] Failed to load page (status: {response.status if response else 'None'})")
    ...
    def _generate_report(self):
        ...
        # Print results
        print(f"Proxy Manager: {status_symbols.get(self.test_results['proxy_test'], '❓')} {self.test_results['proxy_test']}")
        print(f"Captcha Handler: {status_symbols.get(self.test_results['captcha_test'], '❓')} {self.test_results['captcha_test']}")
        print(f"Phoenix MLS Scraper: {status_symbols.get(self.test_results['scraper_test'], '❓')} {self.test_results['scraper_test']}")
