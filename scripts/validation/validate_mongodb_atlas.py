    ...
    async def validate_crud_operations(self) -> bool:
        ...
        try:
            ...
            async with self.db_connection.get_database() as db:
                ...
                delete_result = await collection.delete_one({"property_id": self.sample_property_id})
    ...
    async def validate_schema(self) -> bool:
        ...
        try:
            ...
            async with self.db_connection.get_database() as db:
                ...
                retrieved_doc = await collection.find_one({"property_id": test_property.property_id})
            ...
            self.test_results['schema_validation'] = {
                'success': True,
                'models_tested': ['Property', 'PropertyAddress', 'PropertyFeatures', 'PropertyPrice'],
            ...
            async with self.db_connection.get_database() as db:
                ...
                for i in range(5):
                    ...
                    property_data = Property(
                        ...
                        sources=[DataCollectionMetadata(
                            source=DataSource.PHOENIX_MLS,
                            collector_version="pipeline_test_1.0"
                        )]
                    )
                ...
                print(f"SUCCESS: Pipeline batch insert: {len(insert_result.inserted_ids)} properties")
                ...
                # Query by zipcode
                zipcode_properties = await properties_collection.count_documents({"address.zipcode": "85000"})
                ...
                print(f"SUCCESS: Pipeline cleanup: removed {delete_result.deleted_count} test properties")
    ...
    async def validate_performance(self) -> bool:
        ...
        try:
            ...
            async with self.db_connection.get_database() as db:
                ...
                results = await collection.find({"current_price": {"$gte": 400050}}).to_list(length=None)
                query_duration = (datetime.now() - query_start).total_seconds()
                print(f"SUCCESS: Query performance: found {len(results)} docs in {query_duration:.2f}s")
                
                # Test 3: Update performance
                update_start = datetime.now()
                update_result = await collection.update_many(
                    {"current_price": {"$gte": 400050}},
                    {"$set": {"is_active": False}}
                )
                update_duration = (datetime.now() - update_start).total_seconds()
                print(f"SUCCESS: Update performance: {update_result.modified_count} docs in {update_duration:.2f}s")
                
                # Test 4: Aggregation performance
                agg_start = datetime.now()
                pipeline = [
                    {"$group": {"_id": "$address.zipcode", "avg_price": {"$avg": "$current_price"}, "count": {"$sum": 1}}},
                    {"$sort": {"avg_price": -1}}
                ]
                agg_results = await collection.aggregate(pipeline).to_list(length=None)
                agg_duration = (datetime.now() - agg_start).total_seconds()
                print(f"SUCCESS: Aggregation performance: {len(agg_results)} groups in {agg_duration:.2f}s")
                ...
                performance_acceptable = (
                    insert_duration < 10.0 and  # 10s for 100 inserts
                    query_duration < 5.0 and    # 5s for query
                    update_duration < 5.0 and   # 5s for batch update
                    agg_duration < 10.0          # 10s for aggregation
                )
                
                if performance_acceptable:
                    print("SUCCESS: All performance tests within acceptable limits")
                else:
                    print("WARNING:  Some operations slower than expected (may be normal for free tier)")
