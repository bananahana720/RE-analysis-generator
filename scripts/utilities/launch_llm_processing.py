    ...
    async def validate_all(self) -> bool:
        ...
        with Progress(
            ...
            for name, check_func in checks:
                ...
                table.add_row(name, f"[{status_color}]{status_icon}[/{status_color}]", message)

                self.services_status[name] = {"success": success, "message": message}
...
class ServiceLauncher:
    ...
    def start_service(self) -> bool:
        ...
        try:
            # Build command
            cmd = [sys.executable, "-m", "phoenix_real_estate.collectors.processing.service"]
        ...
        for service, status in validator.services_status.items():
            if not status["success"]:
                if "MongoDB" in service:
                    console.print(
                        "  • Start MongoDB: [cyan]net start MongoDB[/cyan] (run as Administrator)"
                    )
                    ...
                    console.print(
                        f"  • Check port {port}: [cyan]netstat -ano | findstr :{port}[/cyan]"
                    )
