...
class MaricopaAPITester:
    ...
    async def _initialize_session(self):
        ...
        connector = aiohttp.TCPConnector(
            limit=self.max_concurrent, limit_per_host=2, enable_cleanup_closed=True
        )
            ...
            for result in group_results:
                if result.response_data:
                    ...
                    self._extract_fields(
                        result.response_data, all_fields, data_types, nested_structures
                    )

                    # Check for pagination
                    if any(
                        key in result.response_data
                        for key in ["total", "page", "per_page", "pages"]
                    ):
                        pagination_info = {
                            k: v
                            for k, v in result.response_data.items()
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        avg_response_time = sum(r.response_time_ms for r in self.test_results) / max(
            1, len(self.test_results)
        )
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"   Avg Response Time: {report['test_summary']['average_response_time_ms']:.1f}ms"
            )
...
async def main():
    ...
    parser.add_argument(
        "--output-file",
        type=str,
        default="maricopa_api_test_report.json",
