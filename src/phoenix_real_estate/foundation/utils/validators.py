...
class DataValidator:
    ...
    def validate_property_data(self, data: Dict[str, Any], source: Optional[str] = None) -> Tuple[bool, List[str]]:
        ...
        if "year_built" in data and data["year_built"] is not None:
            if not isinstance(data["year_built"], int) or data["year_built"] < 1800 or data["year_built"] > 2100:
