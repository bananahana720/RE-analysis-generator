...
class ProcessingIntegrator:
    ...
    def __init__(
        ...
        email_service: Optional[EmailReportService] = None,
    ) -> None:
        ...
        Args:
            ...
            email_service: Optional email reporting service (will create if not provided)
        """
        self.config = config
        ...
        if config.get_typed("email.enabled", bool, False):
            self.email_service = email_service or EmailReportService(config)
            self.logger.info("Email reporting service initialized")
        else:
            self.logger.debug("Email reporting disabled")
        ...
        self.strict_validation = getattr(
            config, "getattr", lambda k, d: getattr(config.settings, k, d)
    ...
    def _ensure_initialized(self) -> None:
        ...
        if not self._initialized:
            raise RuntimeError(
                "Integrator not initialized. Call initialize() or use as context manager."
            )

    async def process_maricopa_property(
        self, collector: MaricopaAPICollector, property_id: str, save_to_db: bool = True
    ) -> IntegrationResult:
        ...
        result = IntegrationResult(success=False, property_id=property_id, source="maricopa_county")
        ...
        try:
            ...
            processing_result = await self.pipeline.process_json(
                raw_data, "maricopa_county", strict_validation=self.strict_validation
            )
    ...
    async def process_phoenix_mls_property(
        self, scraper: PhoenixMLSScraper, property_id: str, save_to_db: bool = True
    ) -> IntegrationResult:
        ...
        result = IntegrationResult(success=False, property_id=property_id, source="phoenix_mls")
        ...
        try:
            ...
            processing_result = await self.pipeline.process_html(
                html_content, "phoenix_mls", strict_validation=self.strict_validation
            )
    ...
    async def process_maricopa_batch(
        ...
        batch_result = BatchIntegrationResult(
            total_processed=0, successful=0, failed=0, processing_time=0.0
        )
        ...
        try:
            ...
            processing_results = await self.pipeline.process_batch_json(
                raw_properties, "maricopa_county", strict_validation=self.strict_validation
            )
            ...
            for proc_result in processing_results:
                integration_result = IntegrationResult(
                    ...
                    property_id=proc_result.property_data.property_id
                    if proc_result.property_data
                    else "unknown",
        ...
        **kwargs,
        ...
        async for raw_data in collector.stream_properties(**kwargs):
            try:
                ...
                source = (
                    collector.get_source_name()
                    if hasattr(collector, "get_source_name")
                    else "unknown"
                )
                ...
                if source == "maricopa_county":
                    processing_result = await self.pipeline.process_json(
                        raw_data, source, strict_validation=self.strict_validation
                    )
                ...
                result = IntegrationResult(
                    ...
                    property_id=processing_result.property_data.property_id
                    if processing_result.property_data
                    else "unknown",
                    source=source,
                ...
                yield IntegrationResult(
                    success=False, property_id="unknown", source=source, error=str(e)
                )
    ...
    def _convert_to_property_schema(self, property_details: PropertyDetails) -> Dict[str, Any]:
        ...
        source_map = {"maricopa_county": "maricopa_county", "phoenix_mls": "phoenix_mls"}
        ...
        property_type = property_type_map.get(
            (property_details.property_type or "").lower().replace("-", "_"), "other"
        )
        ...
        property_dict = {
            ...
            "metadata": {
                ...
                "extracted_at": property_details.extracted_at.isoformat()
                if property_details.extracted_at
                else None,
    ...
    async def _bulk_save_to_database(self, properties: List[PropertyDetails]) -> int:
        ...
        """
        try:
            property_objects = [self._convert_to_property_schema(prop) for prop in properties]
    ...
    def _update_metrics(
        self, result: IntegrationResult, processing_result: ProcessingResult
    ) -> None:
    ...
    async def send_processing_report(
        ...
        try:
            ...
            report_data = ReportData(
                title=f"Phoenix Real Estate {report_type.title()} Report",
                summary=self._generate_processing_summary(batch_result),
            ...
            if report_type == "daily":
                ...
                return await self.email_service.send_success_summary(
                    batch_result, batch_result.processing_time
                )
            else:
