    ...
    def validate_property_id(property_id: str) -> None:
        ...
        if not ValidationPatterns.PROPERTY_ID_PATTERN.match(property_id.strip()):
            raise ValidationError(
                f"Invalid property ID format: {property_id}", context={"property_id": property_id}
            )
    ...
    def validate_base_url(base_url: str, require_https: bool = True) -> str:
        ...
        if require_https and not ValidationPatterns.HTTPS_URL_PATTERN.match(base_url):
            if ValidationPatterns.HTTP_URL_PATTERN.match(base_url):
                raise ConfigurationError("HTTPS-only communication required, HTTP URLs not allowed")
            else:
    ...
    def validate_required_fields(
        ...
        if missing_fields:
            raise ValidationError(
                ...
                context={"missing_fields": missing_fields, "available_fields": list(data.keys())},
            )
...
class ErrorHandlingUtils:
    ...
    def wrap_error(
        ...
        return error_class(
            f"{operation} failed: {str(error)}", context=safe_context, original_error=error
        )
