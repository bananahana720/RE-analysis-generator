...
class CircuitBreaker:
    ...
    async def execute_async(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if not self.can_execute():
            raise ProcessingError("Circuit breaker is open", error_type=ErrorType.TEMPORARY)
...
class DeadLetterQueue:
    ...
    async def add_failed_item(self, item: Dict[str, Any], error: Exception, attempt_count: int):
        """
    ...
    async def get_items_by_error_type(self, error_type: str) -> List[Dict[str, Any]]:
        ...
        """
        async with self._lock:
            return [item for item in self._queue if item["error_type"] == error_type]
...
class FallbackExtractor:
    ...
    def __init__(self):
        ...
        self.sqft_pattern = re.compile(
            r"([0-9,]+)\s*(?:sq\.?\s*ft\.?|sqft|square\s*feet)", re.IGNORECASE
        )

        self.year_pattern = re.compile(
            r"(?:built|constructed|year).*?(19\d{2}|20\d{2})", re.IGNORECASE
        )

        self.parcel_pattern = re.compile(
            r"(?:APN|Parcel\s*(?:Number|#)?)[:\s#]+([0-9]+[\-A-Za-z0-9]*)", re.IGNORECASE
        )
...
class ErrorRecoveryStrategy:
    ...
    async def handle_error(
        ...
        if add_to_dlq and last_error and "item" in context:
            await self.dead_letter_queue.add_failed_item(context["item"], last_error, attempt)
    ...
    async def handle_error_with_circuit_breaker(
        self, service_name: str, func: Callable, context: Dict[str, Any], **kwargs
    ) -> Any:
        """
