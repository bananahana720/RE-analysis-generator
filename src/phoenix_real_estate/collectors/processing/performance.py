...
class BenchmarkResult:
    ...
    def compare_to(self, other: "BenchmarkResult") -> Dict[str, Any]:
        ...
        avg_improvement = (self_stats["avg_duration"] - other_stats["avg_duration"]) / self_stats["avg_duration"] * 100
        p95_improvement = (self_stats["p95_duration"] - other_stats["p95_duration"]) / self_stats["p95_duration"] * 100
        
        return {
            "avg_improvement_percent": avg_improvement,
            "p95_improvement_percent": p95_improvement,
            "success_rate_change": other_stats["success_rate"] - self_stats["success_rate"],
            "is_improvement": avg_improvement > 0 and other_stats["success_rate"] >= self_stats["success_rate"]
        }
...
class BatchSizeOptimizer:
    ...
    def get_optimal_batch_size(self) -> int:
        ...
        if avg_per_item_duration < self.target_duration * 0.8 and avg_success_rate > self.target_success_rate:
            # Performance is good, can increase batch size
            adjustment = self.adjustment_factor
        elif avg_per_item_duration > self.target_duration * 1.2 or avg_success_rate < self.target_success_rate * 0.9:
        ...
        report = {
            ...
            "trends": {
                "processing_time": {
                    ...
                    "trend": "increasing" if len(processing_times) > 1 and processing_times[-1] > processing_times[0] else "stable"
                },
                "memory_usage": {
            ...
            "recommendations": self.analyze_and_recommend(recent_metrics[-1] if recent_metrics else {}),
