...
class ResourceMonitor:
    ...
    def get_operation_metrics(self, operation_id: str) -> Dict[str, Any]:
        ...
        result = {
            "duration_seconds": operation.get("duration_seconds", time.time() - operation["start_time"]),
            "memory_delta_mb": operation.get("memory_delta_mb", 0)
        }
        
        if "cpu_samples" in operation and operation["cpu_samples"]:
            result["avg_cpu_percent"] = sum(operation["cpu_samples"]) / len(operation["cpu_samples"])
    ...
    async def reserve_resources(self, memory_mb: float, cpu_cores: int, duration_seconds: float) -> str:
    ...
    def get_aggregate_stats(self) -> Dict[str, Any]:
        ...
        if completed_operations > 0:
            durations = [op["duration_seconds"] for op in self._active_operations.values() if "duration_seconds" in op]
