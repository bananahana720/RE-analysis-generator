...
class LRUCache:
    ...
    def __init__(self, max_size: int = 1000, ttl_seconds: float = 86400, max_memory_mb: float = 100):
    ...
    def _put_internal_sync(self, key: str, value: Any) -> None:
        ...
        if size > self.max_memory_bytes:
            logger.warning(f"Item too large for cache: {size} bytes > {self.max_memory_bytes} bytes")
...
class CacheManager:
    ...
    async def set(self, property_data: Dict[str, Any], operation: str, response: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if self._backend_type == "memory":
                ...
                await self._backend.setex(
                    cache_key,
                    int(self.config.ttl_hours * 3600),
                    response_str
                )
    ...
    async def warmup(self, properties: List[Dict[str, Any]], responses: List[Dict[str, Any]], operation: str) -> None:
